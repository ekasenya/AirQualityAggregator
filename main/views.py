        ...
        for substance in Substance.objects.filter(show=True):
            if my_stations == '1':
                ...
                for station in stations:
                    ...
                    if len(values):
                        station_data[station['station_id']] = values
...
class SubstanceAllDetailView(CustomSubstanceDetailView):
    template_name = 'main/substance_detail.html'

    def get_context_data(self, **kwargs):
        context = super(SubstanceAllDetailView, self).get_context_data(**kwargs)
        ...
        for service in AirQService.objects.all():
            ...
            last_measuring_data.append(
                {'service_name': service.name, 'last_datetime': measure.createdate.strftime('%d.%m.%Y %H:%M'),
